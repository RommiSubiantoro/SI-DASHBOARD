...
function ManagerDashboard() {
  ...
  // Pagination logic (setelah filteredUsers)
 ...
 const [currentData, setCurrentData] = useState([]);
  ...
  const fetchUnits = async () => {
    try {
      ...
      let unitsList = unitsSnapshot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      }));

      // ✅ Kalau kosong, seed defaultUnits sekali
      if (unitsList.length === 0) {
        ...
        // Ambil ulang setelah seed
        const newSnapshot = await getDocs(unitsCollection);
        unitsList = newSnapshot.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
        }));
      }
      ...
      // ✅ Set default selected unit kalau belum ada
