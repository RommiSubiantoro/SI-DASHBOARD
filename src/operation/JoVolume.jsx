...
function JoVolume() {
  ...
  const calculateVolumeFromEstimasi = () => {
    ...
    const newVolumeData = {
      ...
      tally: [
        { desc: "Tally", unit: "", values: Array(12).fill(0), total: "0" },
      ],
      erection: [
        {
          desc: "Erection On Base",
  ...
  const exportToExcel = async () => {
    try {
      ...
      // Header Information
      wsData.push(["JO VOLUME TRACKER"]);
      wsData.push(["Unit Bisnis:", selectedUnit]);
      wsData.push(["Tahun:", selectedYear]);
      wsData.push(["Tanggal Export:", new Date().toLocaleString("id-ID")]);
      wsData.push([]);
      ...
      categories.forEach((cat) => {
        // Category title
        wsData.push([cat.title]);

        // Table header
        wsData.push(["Description", "Unit", ...months, "Total"]);
      ...
      XLSX.utils.book_append_sheet(wb, ws, "JO Volume");

      // Generate Excel file and download
      XLSX.writeFile(
        wb,
        `JO_Volume_${selectedUnit.replace(/\s+/g, "_")}_${selectedYear}.xlsx`
      );
    } catch (error) {
    ...
    let htmlContent = `
      ...
      <body>
        ...
        <div class="info">
          ...
          <p><strong>Tanggal Export:</strong> ${new Date().toLocaleString(
            "id-ID"
          )}</p>
        </div>
    `;
  ...
  const renderTable = (category, title, bgColor) => {
    ...
    return (
      <div className="mb-6">
        <h3 className={`${bgColor} font-bold text-sm p-2 mb-2 rounded`}>
          {title}
        </h3>
              ...
              <tr className="bg-gray-200">
                ...
                {isMonthlyView ? (
                  ...
                  months.map((month, idx) => (
                    <th
                      key={idx}
                      className="border border-gray-400 p-2 text-center min-w-[70px]"
                    >
                      {month}
                    </th>
                  ))
                )}
                <th className="border border-gray-400 p-2 text-center bg-yellow-100">
                  Total
                </th>
              </tr>
            </thead>
    ...
    <div className="p-4 bg-white min-h-screen">
      <div className="mb-6 bg-gradient-to-r from-blue-600 to-blue-800 text-white p-4 rounded-lg">
        ...
        <p className="text-sm mt-1 opacity-90">
          Tracking volume operasional berdasarkan data JO Estimasi
        </p>
      </div>
